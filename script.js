...
function displayEmpty(){
    if(Object.keys(obj).length == 0) document.getElementById("empty-para").style.display="block";
...
function callme(){
    ...
    <div class="time time-extra">
        <input id="hour${id}" type="number" value=${hour.value}>
        <p>:</p>
        <input id="minute${id}" type="number" value=${minute.value}>
        <p>:</p>
        <input id="second${id}" type="number" value=${second.value}>
    </div>
    ...
    var total = 0;
    function getTotal(){
        total = Number(currHr.value)*3600 + Number(currMin.value)*60 + Number(currSec.value);
...
function isValid(){
    ...
    if(hr<0 || min <0 || sec<0 || min>60 || sec>60 || hr>23)return false;
    return true;
}